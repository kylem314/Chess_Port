...
for i in board: # for every space in board
    string = str(i) # string is space name
    row = int(string[1]) # row is row number
    row = 9 - row # rows are reversed
    column = (string[0])
    if row == 1:
        list1.insert(0,board[i])  # list1 would now be [board[a8]] cuz that was inserted, and would add the rest each time it iterates thru
    elif row == 2:
        list2.insert(0,board[i])
    elif row == 3:
        list3.insert(0,board[i])
    elif row == 4:
        list4.insert(0,board[i])
    elif row == 5:
        list5.insert(0,board[i])
    elif row == 6:
        list6.insert(0,board[i])
    elif row == 7:
        list7.insert(0,board[i])
    elif row == 8:
        list8.insert(0,board[i])
